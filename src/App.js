...
class BooksApp extends React.Component {
  ...
  setShelves(){
    ...
    this.state.books.map((book) => {
      const a = 0;//Math.floor(Math.random() * 3);

      if (a === 0){
        
        BooksAPI.update(book, 'Currently Reading').then((res) => console.log(res));
      }
      // else if (a === 1){
      //   BooksAPI.update(book, 'Currently Reading').then((res) => this.setState((currentState) => ({books: currentState.books.concat(res)})));
      // }
      // else if (a === 2){
      //   BooksAPI.update(book, 'Currently Reading').then((res) => this.setState((currentState) => ({books: currentState.books.concat(res)})));
      // }
